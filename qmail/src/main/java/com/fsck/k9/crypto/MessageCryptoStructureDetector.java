...
public class MessageCryptoStructureDetector {
    ...
    public static boolean isMultipartEncryptedSMimeProtocol(Part part) {
        if (!isSameMimeType(part.getMimeType(), MULTIPART_ENCRYPTED)) {
            throw new IllegalArgumentException("Part is not multipart/encrypted!");
        }

        String protocolParameter = MimeUtility.getHeaderParameter(part.getContentType(), PROTOCOL_PARAMETER);
        return APPLICATION_SMIME_ENCRYPTED.equalsIgnoreCase(protocolParameter);
    }
    ...
    public static boolean isMultipartSignedSMimeProtocol(Part part) {
        ...
        String protocolParameter = MimeUtility.getHeaderParameter(part.getContentType(), PROTOCOL_PARAMETER);
        return APPLICATION_SMIME_SIGNATURE.equalsIgnoreCase(protocolParameter);
    }
